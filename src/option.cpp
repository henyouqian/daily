...
void setOption(const char* key, int value, bool now){
	_intMap[key] = value;
    if ( now ){
        std::stringstream ss;
        ss << "REPLACE INTO option (key, value) VALUES (\"" << key << "\", " << value << ");";
        int r = sqlite3_exec(g_pSaveDb, ss.str().c_str(), NULL, NULL, NULL);
        lwassert(r == SQLITE_OK);
    }
}
...
void setOptionStr(const char* key, const char* value, bool now){
	_strMap[key] = value;
    if ( now ){
        std::stringstream ss;
        ss << "REPLACE INTO option (key, value) VALUES (" << key << ", " << value << ");";
        sqlite3_exec(g_pSaveDb, ss.str().c_str(), NULL, NULL, NULL);
    }
}
